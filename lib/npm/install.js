...
function install(context, next) {
  ...
  var bailed = false;
  context.emit('data', 'info', context.module.name, 'npm install started');
  ...
  function cleanup() {
    ...
    bailed = true;
    context.emit('data', 'error', context.module.name, 'npm-install Timed Out');
    proc.kill();
  ...
  proc.on('close', function(code) {
    ...
    context.emit('data', 'info', context.module.name, 'npm install successfully completed');
