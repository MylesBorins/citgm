...
function grabProject(context, next) {
  ...
  var bailed = false;
  context.emit('data', 'info',context.module.name,'Downloading project: ' + packageName);
  var proc =
    spawn(
      'npm',
      ['pack', packageName],
  ...
  function cleanup() {
    ...
    bailed = true;
    context.emit('data', 'error', context.module.name, 'Download Timed Out');
    proc.kill();
  ...
  proc.on('close', function(code) {
    ...
    context.emit('data', 'info',context.module.name,'Project downloaded ' + filename);
