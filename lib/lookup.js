...
function filterTags(module, include, exclude) {
  if (include) {
    _.filter(lookup, function(module) {
      return _.any
    });
  }
  if (exclude) {
...
function getLookupTable(options) {
  try {
    ...
    if (options.lookupInclude || options.lookupInclude) {
      lookup = filterTags(lookup, options.lookupInclude, options.lookupInclude);
    }    
    return lookup;
  } catch (err) {
    return undefined;
